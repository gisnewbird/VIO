...
quaterntionsMid = []
timestampMid = []
data = np.loadtxt(filepath + '/midimu_int_pose.txt')
# timestamp1 = data[:,0]
# quaterntions1 = data[:,[tx_index + 6, tx_index + 3, tx_index + 4, tx_index + 5]] # qw,qx,qy,qz
positionMid = data[:,[tx_index, tx_index + 1, tx_index + 2]]
...
ax.plot(position1[:,0], position1[:,1], position1[:,2], label='imu_eular') #描绘欧拉积分结果
ax.plot(positionMid[:,0], positionMid[:,1], positionMid[:,2], label='imu_midPoint') #描绘中值积分结果
